...
版本：v1.2 · 修订日期：2026-09-05 · 状态：已采纳技术审计和产品经理审计意见的设计稿，待业务确认、开发与验收

本版保留 A01—A06 的预算、删除、版本、重试、身份和量化验收约束，按产品经理审计重新划分 V1 范围、估算工期，补充产品事件、体验、合规及功能降级的技术要求。修订关系见第 17 节；所有验收均待实际系统执行。[v1.0 快照](E:/ai-customer-service-plan/versions/AI智能客服系统开发文档-v1.0.md)与[v1.1 快照](E:/ai-customer-service-plan/versions/AI智能客服系统开发文档-v1.1.md)保留历史证据。
...
| 聊天 | 多轮、流式、停止、末轮重新生成及有效答案 | Must |
| 历史记录 | 保存、分页、异步删除和禁止回写；搜索／重命名可延期 | Must 核心；Could 搜索等 |
...
| 基础运维 | 脱敏日志、产品埋点、紧急关闭 AI、备份恢复及他人接手 | Must |
...
同时增加客服工作台、队列、人工回复、内部服务登记、反馈原因和纠错审核、访客凭证及基础防滥用。匿名与已验证业务客户分别授权，协议见第 13.4 节；V2 可先不接业务身份，个人业务查询仍在 V3。公开种子试点前必须通过 PRD 第 7 节 C01—C07 和 G3，完成五个交互流程、功能降级演练与排班；按种子、5%、25%、100% 的证据门槛推进，不按日期自动扩大流量。
...
| 后台任务 | V1 使用数据库持久 job 与 Worker；V2 按需要扩展队列 | V1 执行删除、对账与授权导出；V2 增加解析、向量及运营任务 |
...
V2 内容安全策略覆盖用户输入、检索内容和生成输出，不能只靠系统提示词。流式文本先经过缓冲与策略检查再发布；不能可靠检查增量文本的策略使用完整答复审核，审核未通过或服务不可用时按获批规则阻断并转人工。审核成本和等待时间纳入调用预算与端到端延迟；若使用收费模型审核，也占用 operation/attempt 和 turn 配额。现有 3 次模型上限无法容纳时须在测试前冻结新预算档位，不能暗中增加无限审核调用。
...
v1.2 的工作包和日历以 PRD 第 9 节为唯一估算明细。按一名熟练全栈开发、一个供应商、独立中文网页且业务资料及时到位估算。人日不等于跨岗位可自动并行的日历天，审批与供应商等待另列。

| 阶段 | 估算 | 可验收交付物 |
| --- | --- | --- |
| V0 技术验证 | 2—4 人日 | 一个服务端模型连接、流式测试页、多轮与故障验证 |
| V1 Must 内部系统 | 24—39 人日 | 包含 A01—A04／A06 独立验收 4—6 人日；基础埋点、停用和接手 |
| V1 Should／Could | 分别另加 3—5／3—6 人日 | 评分与完整配置界面／自助导出、搜索和看板，可延期 |
| V2 知识与客服试点 | 24—37 人日 | 包含 A02 扩展、A05、A06、体验和埋点验收 4—6 人日 |
| V3 业务工具 | 接口就绪后独立估算 | 具体接口、身份、权限、确认和幂等验证 |
| V4 训练实验 | 数据与资格核验后独立估算 | 已审核数据集、复现、对比评测和发布建议 |

V0—V2 基础合计 50—80 开发人日，加 20% 缓冲后 60—96；不含可选项和 16—29 非开发人日。原 19—34 天等估算仅存在历史版本，不作为当前承诺。示例最早 2026-09-14 开始、G2 为 2026-10-27—11-24、G3 为 2026-12-04—2027-01-25；仅按周一至周五演算，尚未扣除节假日和外部等待，见 PRD 的计算与前提。
...
> G1 通过后开发内部网页 AI 聊天系统：一个模型连接、登录与会话隔离、多轮流式与末轮重试、历史和安全删除、最小版本及紧急停用、后端预算账本、授权迁移导出、基础埋点和备份接手。评分与配置界面为 Should，自助导出／看板／高级搜索为 Could，完整纠错在 V2。按相应 A01—A06 验收；外部访客和行业能力另过 G3。工期以 v1.2 工作包为准。
...
本版保留 v1.0／v1.1 快照与原审计证据，修改表示规格已采纳，不代表运行时风险已被测试排除。缺少必要配置的功能不得以无限额度或匿名放行方式上线。
...
详见[v1.2 修订记录](E:/ai-customer-service-plan/AI智能客服系统-v1.2修订记录.md)。业务值、合规路径与日历尚待确认，本次未实施应用、发起采购、招募客户或开放服务。
